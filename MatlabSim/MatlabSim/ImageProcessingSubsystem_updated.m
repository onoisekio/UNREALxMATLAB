...
k = 1;
subsample = 15;
someCounter = 1;  % Counting number of surveys
...
elseif State == 2
    St = ImStat == 0;
    ImStat = ImStat + St;  %ensure no zeros
    ImData = ImData + mask;
    ImData = ImData./ImStat;
    block.OutputPort(1).Data = ImData;
    ImStat(:,:) = 0;
    imCounter = 1;
    SummaryData = struct('ImData',[],'ImStat',[]);
     SummaryData.ImData = ImDataArrays;
     SummaryData.ImStat = ImStatArrays;
%      save(strcat('ImDataSummary',num2str(someCounter),'.mat'),"SummaryData");
    ImDataArrays = [];
    ImStatArrays = [];
